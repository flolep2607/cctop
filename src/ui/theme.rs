...
fn detect_light(colorfgbg: Option<&str>) -> bool {
    let Some(bg) = colorfgbg.and_then(|v| v.rsplit(';').next().map(str::trim).and_then(|b| b.parse::<u8>().ok())) else {
        return false;
    };
    bg == 7 || bg >= 9
}
...
mod tests {
    ...
    fn no_color_beats_an_explicit_theme() {
        assert_eq!(select(Some("1"), Some("light"), None).variant, Variant::Mono);
        // Set but empty is not set, per the NO_COLOR convention.
        assert_eq!(select(Some(""), Some("light"), None).variant, Variant::Light);
    }
    ...
    fn only_mono_uses_reset() {
        for p in [DARK, LIGHT] {
            for c in [
                p.border, p.value, p.dim, p.accent, p.cost_low, p.cost_high, p.selected_bg,
                p.header_bg, p.spark_baseline,
            ] {
