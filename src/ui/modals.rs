...
pub(super) fn draw_help(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    let lines = vec![
        ...
        item("", "when its clipboard_control allows reads. Where"),
        item("", "it will not, easiest is the page: `cctop serve`"),
        item("", "here, open it there, and paste the image into"),
        item("", "the box that answers a session — a browser can"),
        item("", "read one where a terminal cannot. Otherwise scp"),
        item("", "the file and type its path, or paste the image"),
        item("", "as base64 text, which cctop also files:"),
