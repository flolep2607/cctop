...
pub fn run(args: &Args, hosted: Option<crate::shim::Hosted>) -> anyhow::Result<i32> {
    ...
    // A paste is one event rather than thousands of key events. This matters
    // most inside an attached agent pane, where every individual key otherwise
    // has to cross cctop's shim socket before the agent can read it.
