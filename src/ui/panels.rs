            ...
            for line in wrap(full, width.saturating_sub(10)) {
                out.push(
                    Line::from(vec![
                        ...
                        Span::styled(
                            line,
                            Style::default().fg(theme::gray(252)),
                        ),
                    ])
                    .style(row_style),
                );
...
pub fn context(session: &Session, data: Option<&SessionData>, width: usize) -> Vec<Line<'static>> {
    ...
    if unaccounted > 0 {
        slices.push(Slice::held("Unaccounted", unaccounted as u64, theme::colors().dim));
    }
...
fn missing(text: String) -> Line<'static> {
    Line::from(Span::styled(text, Style::default().fg(theme::colors().dimmer)))
}
...
fn skill_list(dir: &Path) -> Vec<Line<'static>> {
    ...
    for entry in crate::config::list_dir(dir) {
        ...
        let mut spans = vec![Span::styled(name, Style::default().fg(theme::colors().cost_low))];
        if !desc.is_empty() {
            spans.push(Span::raw("  "));
